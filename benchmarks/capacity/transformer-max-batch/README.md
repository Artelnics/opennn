...
```bash
# 1. Build the OpenNN trial (registered in benchmarks/CMakeLists.txt).
...
python benchmarks/energy/transformer-energy/prepare_chat.py
# 2b. WMT14 En-De corpus (standard, publishable):
python benchmarks/capacity/transformer-max-batch/prepare_wmt14.py

# 3. Run the full comparison (torch + TF live in the ml venv).
#    Alpaca (train + infer):
python benchmarks/capacity/transformer-max-batch/run_transformer_maxbatch.py \
...
python benchmarks/capacity/transformer-max-batch/run_transformer_maxbatch.py \
    ...
    --result-json benchmarks/results/gpu-transformer-max-batch-wmt14.json
```
