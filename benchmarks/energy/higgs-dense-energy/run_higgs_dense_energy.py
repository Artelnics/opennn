#!/usr/bin/env python3
"""GPU fixed-work energy benchmark: HIGGS dense classifier, OpenNN vs PyTorch vs TensorFlow.

Every engine trains the IDENTICAL canonical HIGGS dense classifier
(28 -> hidden -> hidden -> 1, ReLU hidden, sigmoid output, binary cross entropy)
for the IDENTICAL fixed work (same epochs, batch, shuffle protocol, Adam
defaults) on the IDENTICAL prepared CSVs, using the engine drivers of the
dense speed track (../../throughput/higgs-gpu). What differs is the engine and
its fastest execution path (OpenNN GPU-resident data + CUDA graph, PyTorch
GPU-resident tensors, TensorFlow XLA train step), so energy-per-sample is an
engine comparison at equal work, not a hyperparameter lottery.
...
For every run we:
  ...
  * integrate (trapezoid) ONLY between the TRAIN_START_UNIX / TRAIN_END_UNIX
    markers each engine prints around its timed training loop -- warmup epochs
    and one-time CSV loading stay outside the window,
  * report total and active energy (idle baseline subtracted), microjoules per
    nominal epoch-sample (energy / (train_rows x epochs), same divisor for
    every engine), average power, median SM clock, wall time, and the speed /
    quality metrics the engine prints.
...
power, not a HW joule counter). Run on a quiet GPU.
  ...
  usage: run_higgs_dense_energy.py [--train CSV] [--test CSV] [--epochs N]
                                   [--batch N] [--hidden N] [--hidden-layers N]
                                   [--activation relu|tanh] [--precision fp32|bf16|both]
                                   [--runs N] [--engines opennn,pytorch,tensorflow]
"""
...
DRIVERS = (HERE.parent.parent / "throughput" / "higgs-gpu").resolve()
...
def find_opennn_speed() -> tuple[str, bool]:
    ...
    names = ["opennn_speed.exe", "opennn_speed"] if os.name == "nt" else ["opennn_speed", "opennn_speed.exe"]
    dirs = [
        DRIVERS,
        REPO_ROOT / "build" / "bin",
        REPO_ROOT / "build" / "bin" / "Release",
        REPO_ROOT / "build-gpu" / "bin",
        REPO_ROOT / "build-bench" / "bin",
        REPO_ROOT / "build-benchmarks" / "bin",
        REPO_ROOT / "build-benchmarks" / "bin" / "Release",
        Path.home() / "opennn-wsl" / "build-bench" / "bin",
...
def cmd_env(engine: str, precision: str):
    ...
    if engine == "opennn":
        cmd = [OPENNN_BIN, args.train, str(args.epochs), str(args.batch), precision,
               str(args.hidden), args.activation, str(args.hidden_layers), args.test,
               *thresholds]
        if Path(wsl_cuda).exists():
            env["LD_LIBRARY_PATH"] = wsl_cuda + os.pathsep + env.get("LD_LIBRARY_PATH", "")
    elif engine in ("pytorch", "tensorflow"):
        cmd = [PY, str(DRIVERS / f"{engine}_speed.py"), args.train, str(args.epochs),
               str(args.batch), precision, args.shuffle, str(args.hidden),
               args.activation, str(args.hidden_layers), args.test, *thresholds]
...
def run_one(engine, precision, idle_w, trace_path):
    ...
    m = {
        ...
        "train_rows": parse_marker(r"^samples=(\d+)", out, int),
        "samples_per_sec": parse_marker(r"^samples_per_sec=([0-9.]+)", out),
        "median_epoch_s": parse_marker(r"^median_epoch_s=([0-9.eE+-]+)", out),
        "test_accuracy": parse_marker(r"^test_accuracy=([0-9.eE+-]+)", out),
        "test_log_loss": parse_marker(r"^test_log_loss=([0-9.eE+-]+)", out),
        "test_roc_auc": parse_marker(r"^test_roc_auc=([0-9.eE+-]+)", out),
        "quality_gate": parse_marker(r"^quality_gate=(\w+)", out, str),
    }
    ...
    if train_start and train_end and samples:
        ...
        if m["train_rows"]:
            nominal = m["train_rows"] * args.epochs
...
def main():
    ...
    ap.add_argument("--train", default=str(BENCH_DATA / "higgs" / "higgs_train.csv"))
    ap.add_argument("--test", default=str(BENCH_DATA / "higgs" / "higgs_test.csv"))
    ap.add_argument("--epochs", type=int, default=20,
                    help="fixed work: identical timed epochs for every engine")
    ap.add_argument("--batch", type=int, default=7000)
    ap.add_argument("--hidden", type=int, default=1024)
    ap.add_argument("--hidden-layers", type=int, default=2)
    ap.add_argument("--activation", default="relu", choices=["relu", "tanh"])
    ap.add_argument("--shuffle", default="shuffle", choices=["shuffle", "noshuffle"])
    ap.add_argument("--precision", default="fp32", choices=["fp32", "bf16", "both"])
    ap.add_argument("--runs", type=int, default=3)
    ap.add_argument("--engines", default="opennn,pytorch,tensorflow")
    ap.add_argument("--min-accuracy", default=os.environ.get("HIGGS_MIN_ACCURACY"))
    ap.add_argument("--max-log-loss", default=os.environ.get("HIGGS_MAX_LOG_LOSS"))
    ap.add_argument("--min-auc", default=os.environ.get("HIGGS_MIN_AUC"))
    ...
    result = {
        "schema_version": 1,
        "benchmark_id": "gpu-higgs-dense-energy",
        "run_id": run_id,
        "git_commit": git_metadata()["commit"],
        "git": git_metadata(),
        "dataset": "HIGGS",
        "configuration": {
            "task": "canonical HIGGS dense classifier trained for fixed work "
                    "(identical epochs/batch/model for every engine)",
            "model": f"28 -> {args.hidden} x{args.hidden_layers} ({args.activation}) -> 1 sigmoid, BCE, Adam",
            ...
            "note": "GPU energy only (board sensor); sampled power, not a HW joule "
                    "counter; uj_per_sample uses nominal train_rows x epochs, the "
                    "same divisor for every engine",
        },
        "machine": versions(),
        "runner": {
    ...
    for precision in precisions:
        ...
        for eng in engines:
            ...
            for r in range(args.runs):
                cooldown(idle_w)
                trace = RESULTS_DIR / f".trace-higgs-energy-{eng}-{precision}-{run_id}-{r}.csv"
                ok, m, out = run_one(eng, precision, idle_w, trace)
                trace.unlink(missing_ok=True)
                if not ok:
                    print(f"  run {r}: FAILED rc={m['rc']} gate={m.get('quality_gate')}")
            ...
            if per_run:
                for key in ("energy_total_j", "energy_active_j", "uj_per_sample_total",
                            ...
                            "test_accuracy", "test_log_loss", "test_roc_auc"):
