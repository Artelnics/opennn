//   OpenNN Transformer TRAINING throughput, GPU. Fair counterpart to the
//   PyTorch/TensorFlow training scripts: same architecture, same synthetic
//   corpus (token-for-token), same optimizer (Adam) and loss (token
//   cross-entropy over the vocabulary).
//
//   One train() call runs 1 + epochs epochs and post_epoch_callback stamps the
//   clock at every epoch boundary. The first epoch is the untimed warmup (CUDA
//   context, cuBLASLt / cuDNN plan caches, allocator, graph capture -- the
//   counterpart of the excluded warmup epoch in the PyTorch/TensorFlow
//   scripts); the median of the timed epochs gives samples/sec and tokens/sec,
//   the steady-state forward+backward+update throughput of the resident GPU path.
//
...
//          OPENNN_TRANSFORMER_TRAIN_NO_GRAPH=1 -> train eagerly, without the CUDA graph
...
#include "benchmarks/transformer_benchmark.h"
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        const bool use_cuda_graph = getenv("OPENNN_TRANSFORMER_TRAIN_NO_GRAPH") == nullptr;
        const float lr = getenv("OPENNN_LR") ? stof(getenv("OPENNN_LR")) : 0.0001f;

        adam->set_batch_size(batch);
        adam->set_cuda_graph(use_cuda_graph);
        ...
        cout << "cuda_graph=" << (!use_cuda_graph ? "off"
