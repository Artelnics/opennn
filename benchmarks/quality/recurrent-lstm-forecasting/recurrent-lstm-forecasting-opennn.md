...
| Artifact rule | JSON under `benchmarks/results/` with protocol metadata, command, machine data, parsed metrics, and raw output |
...
```bash
cd benchmarks/quality/recurrent-lstm-forecasting
...
```text
benchmarks/results/recurrent-lstm-forecasting-<run_id>.json
```
