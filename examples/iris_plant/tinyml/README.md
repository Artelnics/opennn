...
Proves that a model trained with OpenNN and exported to plain C
(`ModelExpression::save(..., ProgrammingLanguage::C)`) produces the same
outputs when compiled for an Arduino-class microcontroller (ATmega328P /
...
- `iris_model.c` compiles unmodified with `avr-gcc -mmcu=atmega328p`
  (`OPENNN_EXPORT_NO_MAIN` removes the demo `main()`; inference uses no heap).
- The firmware fits the ATmega328P: ~6.6 KB of 32 KB flash, ~190 B of 2 KB RAM
  (4-16-3 classification MLP).
...
Measured on 2026-07-06 (avr-gcc 7.3.0, simavr 1.6, WSL2 Ubuntu 24.04):
    ...
    OpenNN reference vs exported C on PC : max abs diff = 5.985e-08
    OpenNN reference vs exported C on AVR: max abs diff = 8.948e-08
    Exported C: PC vs AVR emulator       : max abs diff = 8.941e-08
    Predicted class agreement (AVR vs reference): 9/9
    Flash: 6602 B / 32 KB — RAM (data+bss): 186 B / 2 KB
