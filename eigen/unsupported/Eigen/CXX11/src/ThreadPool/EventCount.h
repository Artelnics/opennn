...
class EventCount {
 public:
  ...
  void CancelWait() {
    ...
    for (;;) {
      ...
      // Only if number of waiters is equal to number of signalsCount,
  ...
  void Notify(bool notifyAll) {
    ...
    for (;;) {
      ...
      const uint64_t signalsCount = (state & kSignalMask) >> kSignalShift;
      // Easy case: no waiters.
      if ((state & kStackMask) == kStackMask && waiters == signalsCount) return;
      uint64_t newstate;
      if (notifyAll) {
        // Empty wait stack and set signal to number of pre-wait threads.
        newstate =
            (state & kWaiterMask) | (waiters << kSignalShift) | kStackMask;
      } else if (signalsCount < waiters) {
      ...
      if (state_.compare_exchange_weak(state, newstate,
                                       std::memory_order_acq_rel)) {
        if (!notifyAll && (signalsCount < waiters))
 ...
 private:
  ...
  // - next kWaiterBits is count of pending signalsCount.
  ...
  static void CheckState(uint64_t state, bool waiter = false) {
    ...
    const uint64_t waiters = (state & kWaiterMask) >> kWaiterShift;
    const uint64_t signalsCount = (state & kSignalMask) >> kSignalShift;
    eigen_plain_assert(waiters >= signalsCount);
