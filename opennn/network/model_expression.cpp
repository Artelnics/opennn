...
static constexpr const char* javascript_header =
    ...
    "Artificial Intelligence Techniques SL\n"
    "artelnics@artelnics.com\n\n"
    "Model exported to JavaScript. Form sliders set inputs; click the button to run network().\n\n"
    "Input Names:";
...
<form onsubmit="network(); return false;">
<h4>INPUTS</h4>
...
static constexpr const char* python_header =
    ...
    "Artificial Intelligence Techniques SL\n"
    "artelnics@artelnics.com\n\n"
    "Model exported to Python. Use Network().calculate_outputs([...]).\n\n"
    "Input Names: \n";
...
ModelExpression::ModelExpression(const Network* network) : network(network) {}

vector<string> ModelExpression::get_flat_input_names() const
{
    vector<string> names = network->get_input_feature_names();
    const Index flat_inputs_number = network->get_inputs_number();
...
ModelExpression::ExportNames ModelExpression::collect_names() const
{
    ...
    names.outputs = network->get_output_feature_names();
...
{
    if (network->get_parameters_device() != Device::CPU)
        return;

    const ConstVectorMap parameters = network->get_parameters_map();
...
{
    return build_expression(get_flat_input_names(), network->get_output_feature_names());
}

string ModelExpression::build_expression(const vector<string>& input_names,
                                         const vector<string>& output_names) const
{
    const Network::HostParametersGuard host_parameters(
        *const_cast<Network*>(network));

    const size_t layers_number = network->get_layers_number();
    const vector<string> layer_labels = network->get_layer_labels();
...
string ModelExpression::get_expression_c_embedded() const
{
...

    {
        ...
        throw_if(layers_number == 0,
                 "ModelExpression: the network is empty.");
        ...
        array<bool, 2> buffer_used{};

        Index max_width = inputs_number;
        Index max_hidden = 0;

        string_view current = "inputs";

        constexpr array<string_view, 2> buffer_names{
            "nn_buffer_a",
            "nn_buffer_b"
        };
        ...
        const auto emit_float_array =
            [&](const string& name, const span<const float> values)
        {
            tables << "static const float "
                   << name << "[" << values.size() << "] NN_FLASH = {";
            ...
            for(size_t i = 0; i < values.size(); ++i)
            {
                ...
                tables << c_float_literal(values[i]);
        ...
        const auto emit_byte_array =
            [&](const string& name, const span<const unsigned char> values)
        {
            tables << "static const unsigned char "
                   << name << "[" << values.size() << "] NN_FLASH = {";
        ...
        const auto take_buffer = [&]() -> string_view
        {
            buffer_used[next_buffer] = true;

            const string_view target = buffer_names[next_buffer];
        ...
        const auto in_place_target = [&]() -> string_view
        {
            if(current == "inputs")
            {
                current = take_buffer();
        ...
        for(size_t i = 0; i < layers_number; ++i)
        {
            ...
            const bool is_last = i + 1 == layers_number;

            if(is_one_of(layer_type,
                         LayerType::Scaling,
                         LayerType::Unscaling))
            {
                const Scaling* scaling =
                    static_cast<const Scaling*>(layers[i].get());

                const vector<Descriptives>& descriptives =
                    scaling->get_descriptives();
                ...
                vector<float> slopes(static_cast<size_t>(features_number));
                vector<float> offsets(static_cast<size_t>(features_number));

                vector<unsigned char> log_pre(
                    size_t(features_number), 0);

                vector<unsigned char> exp_post(
                    size_t(features_number), 0);
                ...
                const string_view target = take_buffer();
                ...
                if(any_flag)
                {
                    ...
                    emit_byte_array(
                        table_prefix + "_log_pre",
                        log_pre);

                    emit_byte_array(
                        table_prefix + "_exp_post",
                        exp_post);

                    body << "\tnn_affine_flags_forward("
                         << current << ", "
                         << table_prefix << "_log_pre, "
                         << table_prefix << "_a, "
                         << table_prefix << "_b, "
            ...
            else if(layer_type == LayerType::Dense)
            {
                ...
                const Index layer_inputs =
                    dense->get_inputs_number();

                const Index layer_outputs =
                    dense->get_outputs_number();

                const float* bias_data =
                    parameter_views[0].as<float>();

                const float* weight_data =
                    parameter_views[1].as<float>();

                emit_float_array(
                    table_prefix + "_weights",
                    span<const float>(
                        weight_data,
                        size_t(layer_inputs * layer_outputs)));

                emit_float_array(
                    table_prefix + "_biases",
                    span<const float>(
                        bias_data,
                        size_t(layer_outputs)));

                const ActivationFunction activation =
                    dense->get_activation_function();
            ...
            else if(layer_type == LayerType::Recurrent)
            {
                const Recurrent* recurrent =
                    static_cast<const Recurrent*>(layers[i].get());

                const vector<TensorView>& parameter_views =
                    recurrent->get_parameter_views();
                ...
                const Shape input_shape =
                    recurrent->get_input_shape();

                const Shape output_shape =
                    recurrent->get_output_shape();
                ...
                const MatrixMap input_w_map =
                    parameter_views[1].as_matrix();
                ...
                const string_view activation_constant =
                    activation_constant_for(
                        ActivationOperator::from_string(
                            recurrent->get_activation_function()));
                ...
                body << "\tnn_recurrent_forward("
                     ...
                     << hidden << ", "
                     << activation_constant << ", "
                     << (return_sequences ? 1 : 0)
                     << ", nn_state_a, nn_state_b, "
                     << target << ");\n";
            ...
            else if(layer_type == LayerType::LongShortTermMemory)
            {
                ...
                const vector<TensorView>& parameter_views =
                    lstm->get_parameter_views();

                const bool configured =
                    parameter_views.size() >= 12
                    && ranges::all_of(
                ...
                const Index features =
                    lstm->get_input_features();

                const Index hidden =
                    lstm->get_output_features();

                const bool return_sequences =
                    lstm->get_return_sequences();

                vector<float> lstm_biases(
                    size_t(4 * hidden));

                vector<float> lstm_input_weights(
                    size_t(4 * features * hidden));

                vector<float> lstm_recurrent_weights(
                    size_t(4 * hidden * hidden));

                for(Index gate = 0; gate < 4; ++gate)
                {
                    const VectorMap gate_biases =
                        parameter_views[size_t(gate)].as_vector();

                    const MatrixMap gate_w =
                        parameter_views[size_t(4 + gate)].as_matrix();
                ...
                const string_view activation_constant =
                    activation_constant_for(
                        lstm->get_activation_function());

                const string_view recurrent_activation_constant =
                    activation_constant_for(
                        lstm->get_recurrent_activation_function());
                ...
                body << "\tnn_lstm_forward("
                     ...
                     << hidden << ", "
                     << activation_constant << ", "
                     << recurrent_activation_constant << ", "
                     << (return_sequences ? 1 : 0)
                     << ", nn_state_a, nn_state_b, nn_cell, "
                     << target << ");\n";
            ...
            else
            {
                throw runtime_error(
                    format(
                        "ModelExpression: layer '{}' ({}) is not supported in embedded export.",
                        layer_labels[i],
                        layer_type_map().to_string(layer_type)));
            }
        }

        throw_if(
            current == "inputs",
            "ModelExpression: the network has no layers to export.");
        ...
        {
            buffer << "\tstatic float nn_state_a["
                   << max_hidden << "];\n"
                   << "\tstatic float nn_state_b["
                   << max_hidden << "];\n";

            if(uses_lstm)
                buffer << "\tstatic float nn_cell["
                       << max_hidden << "];\n";
        }
...
void ModelExpression::emit_js_inputs_html(ostringstream& buffer, const ExportNames& names) const
{
    ...
    const Scaling* scaling_layer = network->has(LayerType::Scaling)
        ? static_cast<const Scaling*>(network->get_first(LayerType::Scaling))
        : nullptr;
    ...
    const auto& variables = network->get_input_variables();
...
void ModelExpression::emit_js_outputs_html(ostringstream& buffer, bool use_category_select, const ExportNames& names) const
{
    ...
    buffer << "<!-- BUTTON HERE -->\n"
           << "<button type=\"button\" class=\"btn\" onclick=\"network()\">calculate outputs</button>\n"
           << "<br/>\n\n"
...
string ModelExpression::get_expression_python() const
{
    ...
    string expression = build_expression();
    for (const string& name : network->get_output_feature_names())
