...
public:
    ...
    Vector<double> calculate_quarters(void) const;
...
template <class T>
Vector<size_t> Vector<T>::calculate_total_frequencies(const Vector< Histogram<T> >& histograms) const
{
...
// Vector<size_t> calculate_total_frequencies_missing_values(const Vector<size_t> missing_values, const Vector< Histogram<T> >&);
...
template <class T>
Vector<size_t> Vector<T>::calculate_total_frequencies_missing_values(const Vector<size_t> instance_missing_values, const Vector< Histogram<T> >& histograms) const
{
    const size_t histograms_number = histograms.size();
...
template <class T>
Vector<double> Vector<T>::calculate_quarters(void) const
{
    const size_t this_size = this->size();
    ...
    Vector<double> quarters(4);

    if(this_size % 2 == 0)
    {
        quarters[0] = (sorted_vector[this_size/4] + sorted_vector[this_size/4 + 1])/2;
        quarters[1] = (sorted_vector[this_size*2/4] + sorted_vector[this_size*2/4 + 1])/2;
        quarters[2] = (sorted_vector[this_size*3/4] + sorted_vector[this_size*3/4 + 1])/2;
        quarters[3] = calculate_maximum();
    }
    else
    {
        quarters[0] = sorted_vector[this_size/4 + 1];
        quarters[1] = sorted_vector[this_size*2/4 + 1];
        quarters[2] = sorted_vector[this_size*3/4 + 1];
        quarters[3] = calculate_maximum();
    }
...
template <class T>
double Vector<T>::calculate_kurtosis_missing_values(const Vector<size_t>& missing_indices) const
{
    const size_t this_size = this->size();
...
template<class T>
Vector<double> Vector<T>::calculate_shape_parameters_missing_values(const Vector<size_t>& missing_values) const
{
    // Control sentence (if debug)
...
Vector<double> Vector<T>::calculate_box_and_whiskers(void) const
{
    ...
    Vector<double> quarters = calculate_quarters();

    box_and_whispers[0] = calculate_minimum();
    box_and_whispers[1] = quarters[0];
    box_and_whispers[2] = quarters[1];
    box_and_whispers[3] = quarters[2];
    box_and_whispers[4] = quarters[3];
...
template <class T>
Vector<double> Vector<T>::calculate_autocorrelation(const size_t& lags_number) const
{
    ...
    for(size_t i = 0; i < lags_number; i++)
    {
        ...
        {
            numerator += (((*this)[j] - mean)*((*this)[j + i] - mean))/(this_size - i);
        }
        for(size_t j = 0; j < this_size; j++)
        {
...
// Vector<double> calculate_cross_correlation(cosnt Vector<double>&, const size_t&) const
...
template<class T>
Vector<double> Vector<T>::calculate_cross_correlation(const Vector<double>& other, const size_t& maximum_lags_number) const
{
...
/// Saves to a file the minimum, maximum, standard deviation, asymmetry and kurtosis values
...
size_t Histogram<T>::calculate_bin(const T& value) const
{
    ...
    const double length = (double) (maximum_center - minimum_center)/ (double) (bins_number - 1);
