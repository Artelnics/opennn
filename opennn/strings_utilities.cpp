...
/*
    ...
    return tokens;
*/
    // @todo Check. Generated by AI.

    vector<string> tokens;
    string_view str_view(text);
    string_view sep_view(separator);

    string::size_type start = 0;

    while (start < str_view.length())
    {
        // Find the start of the next token
        auto token_start_pos = str_view.find_first_not_of(sep_view, start);

        if (token_start_pos == string_view::npos)
            break; // No more tokens

        // Find the end of the current token
        auto token_end_pos = str_view.find_first_of(sep_view, token_start_pos);

        // Extract the token as a string_view (no allocation)
        string_view token_view = (token_end_pos == string_view::npos)
                                     ? str_view.substr(token_start_pos)
                                     : str_view.substr(token_start_pos, token_end_pos - token_start_pos);

        // Create the final string and add it to the vector
        tokens.emplace_back(token_view);

        if (token_end_pos == string_view::npos)
            break;
