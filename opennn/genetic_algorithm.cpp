...
void GeneticAlgorithm::initialize_population_correlations()
{
    ...
    for(Index i = 0; i < individuals_number; i++)
    {
        ...
        raw_variables_active = 1 /* + arc4random() % input_raw_variables_number*/; //arc4random: identifier not found
        ...
        if(is_equal(individual_raw_variables, false))
            individual_raw_variables(1/*arc4random() % input_raw_variables_number*/) = true; //arc4random: identifier not found
...
void GeneticAlgorithm::perform_crossover()
{
    ...
    for(size_t i = 0; i < parent_1_indices.size(); i++)
    {
        ...
        for(Index j = 0; j < 2; j++)
        {
            ...
            for(Index k = 0; k < raw_variables_number; k++)
                descendent_raw_variables(k) = (1/*arc4random() % 2*/) ? parent_1_raw_genes(k) : parent_2_raw_genes(k); //arc4random: identifier not found
