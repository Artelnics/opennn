...
const Matrix<double>& GeneticAlgorithm::get_population(void) const
{
    return(population);
}
...
const size_t& GeneticAlgorithm::get_maximum_generalization_failures(void) const
{
...
// const bool& get_reserve_generation_minimum(void) const method

/// Returns true if the generation minimum generalization error are to be reserved, and false otherwise.

const bool& GeneticAlgorithm::get_reserve_generation_minimum(void) const
{
    return(reserve_generation_minimum);
}
...
void GeneticAlgorithm::set_default(void)
{
    ...
    {
        maximum_generalization_failures = 10;
    ...
    }else
    {
        ...
        maximum_generalization_failures = (size_t)std::max(10.,inputs_number/5.);
    ...
    reserve_generation_standard_deviation = true;

    reserve_generation_minimum = true;
...
void GeneticAlgorithm::set_population(const Matrix<double>& new_population)
{
    ...
    if (new_population.get_columns_number() != inputs_number)
    {
        std::ostringstream buffer;

        buffer << "OpenNN Exception: GeneticAlgorithm class.\n"
               << "void set_population(const Matrix<double>&) method.\n"
               << "Population columns("<<new_population.get_columns_number()<< ") must be equal to inputs number("<<inputs_number<<").\n";
    ...
    if (new_population.get_rows_number() != population_size)
    {
        ...
        buffer << "OpenNN Exception: GeneticAlgorithm class.\n"
               << "void set_population(const Matrix<double>&) method.\n"
               << "Population rows("<<new_population.get_rows_number()<< ") must be equal to population size("<<population_size<<").\n";
...
// void set_performance(const Matrix<double>&) method

/// Sets a new training performances of the population.
/// @param new_performance New training performances.

void GeneticAlgorithm::set_performance(const Matrix<double>& new_performance)
{
#ifdef __OPENNN_DEBUG__

    if (new_performance.get_columns_number() != 2)
    {
        std::ostringstream buffer;
    ...
    if (new_performance.get_rows_number() != population_size)
    {
        std::ostringstream buffer;

        buffer << "OpenNN Exception: GeneticAlgorithm class.\n"
               << "void set_performance(const Matrix<double>&&) method.\n"
               << "Performance rows("<<new_performance.get_rows_number()<< ") must be equal to population size("<<population_size<<").\n";
    ...
    performance.set(new_performance);
}
...
/// @param new_crossover_method Method to perform the crossover of the selected population(UniformCrossover, Point1 or Points2).
...
{
    if (new_crossover_method == "UniformCrossover")
    {
        crossover_method = UniformCrossover;

    }else if (new_crossover_method == "Point1")
    {
        crossover_method = Point1;

    }else if (new_crossover_method == "Points2")
    {
        crossover_method = Points2;
...
/// Sets the point of for the one-point and two-point crossover.
/// If it is set to 0, the algorithm will select it randomly for each crossover.
/// @param new_crossover_first_point Point for the one-point and first point for two-point crossover.
...
/// Sets the point of for the two-point crossover.
/// If it is set to 0, the algorithm will select it randomly for each crossover.
/// @param new_crossover_second_point Second point for the two-point crossover.
...
void GeneticAlgorithm::set_maximum_generalization_failures(const size_t& new_maximum_performance_failures)
{
#ifdef __OPENNN_DEBUG__
...
// void set_reserve_generation_minimum(const bool&) method

/// Sets the reserve flag for the generation minimum generalization error history.
/// @param new_reserve_generation_minimum Flag value.

void GeneticAlgorithm::set_reserve_generation_minimum(const bool& new_reserve_generation_minimum)
{
    reserve_generation_minimum = new_reserve_generation_minimum;
}
...
void GeneticAlgorithm::initialize_population(void)
{
    ...
    population.set(population_size,inputs_number);
...
{
    const PerformanceFunctional* performance_functiona_pointer = training_strategy_pointer->get_performance_functional_pointer();
    const NeuralNetwork* neural_network_pointer = performance_functiona_pointer->get_neural_network_pointer();
    ...
    for (size_t i = 0; i < population_size; i++)
    {
        ...
        if (zero_ocurrences == inputs_number)
            inputs[rand()%inputs_number] = 1;

        population.set_row(i,inputs);
    }
}
...
void GeneticAlgorithm::initialize_weighted_population(void)
{
    ...
    Vector<double> inputs(inputs_number, 0);
...
/// Perform the trainings of the neural network and calculate its fitness.
...
void GeneticAlgorithm::evaluate_population(void)
{
    ...
    Vector<double> current_inputs;
    size_t current_inputs_number;

    size_t index;
    Vector<double> errors(2);

    performance.set(population_size,2);

    for (size_t i = 0; i < population_size; i++)
    {
        current_inputs = population.arrange_row(i);
        current_inputs_number = current_inputs.count_occurrences(1);

        for (size_t j = 0; j < current_inputs.size(); j++)
        {
            index = get_input_index(original_uses,j);

            if (current_inputs[j] == 0)
            {
        ...
        errors = calculate_performances(population.arrange_row(i));

        performance.set_row(i, errors);
    }
...
void GeneticAlgorithm::calculate_fitness(void)
{
    ...
    if (performance.get_rows_number() == 0)
    {
        std::ostringstream buffer;
...
void GeneticAlgorithm::calculate_objetive_fitness(void)
{
    ...
    fitness.set(performance.get_rows_number(),0.);

    for (size_t i = 0; i < fitness.size(); i++)
        fitness[i] = 1/(1 + performance(i,1));
}
...
{
    const Vector<size_t> rank = performance.arrange_column(1).calculate_greater_rank();
...
void GeneticAlgorithm::evolve_population(void)
{
    ...
    for(size_t i = 0; i < population.get_rows_number(); i++)
    {
        ...
        if (zero_ocurrences == population.get_columns_number())
            population(i,rand()%population.get_columns_number()) = 1;
    }
...
void GeneticAlgorithm::perform_selection(void)
{
    ...
    Matrix<double> population_copy;

    Vector<bool> selected_population(population.get_rows_number(),false);
    ...
    population_copy.set(1,population.get_columns_number());
    ...
    if (elitism_size >= 1)
    {
        ...
        population_copy.append_row(population.arrange_row(selected_index));

        fitness_copy[selected_index] = -1;
    }

    for (size_t i = 1; i < elitism_size && i < selected_population_size; i++)
    {
    ...
    population_copy.subtract_row(0);
...
void GeneticAlgorithm::perform_crossover(void)
{
    ...
    if (population.get_rows_number() <= 1)
    {
        std::ostringstream buffer;
...
/// Perform the one-point crossover method.

void GeneticAlgorithm::perform_1point_crossover(void)
{
    const size_t inputs_number = population.get_columns_number();
    const size_t selected_population = population.get_rows_number();

    size_t parent1_index;
    Vector<double> parent1(inputs_number);
    size_t parent2_index;
    Vector<double> parent2(inputs_number);

    Vector<double> offspring1(inputs_number);
    Vector<double> offspring2(inputs_number);

    size_t random_loops = 0;

    size_t first_point = crossover_first_point;

    while(population.get_rows_number() < population_size)
    {
        ...
        parent1 = population.arrange_row(parent1_index);
        parent2 = population.arrange_row(parent2_index);
        ...
        population.append_row(offspring1);
        if (population.get_rows_number() != population_size)
            population.append_row(offspring2);
    }
}

// void perform_2point_crossover(void) method

/// Perform the two-point crossover method.
...
void GeneticAlgorithm::perform_mutation(void)
{
    ...
    for (size_t i = selected_population_size; i < population.get_rows_number(); i++)
        for (size_t j = 0; j < population.get_columns_number(); j++)
        {
            ...
            {
                population(i,j) = ((size_t)population(i,j) + 1)%2;
            }
        }
}

// size_t GeneticAlgorithm::get_optimal_individual_index(void) const method
...
size_t GeneticAlgorithm::get_optimal_individual_index(void) const
{
    ...
    Vector<double> optimal_inputs = population.arrange_row(0);

    double optimum_error = performance(0,1);
    ...
    {
        current_inputs = population.arrange_row(i);
        current_error = performance(i,1);

        if ((fabs(optimum_error-current_error) < tolerance &&
             current_inputs.count_occurrences(1) > optimal_inputs.count_occurrences(1)) ||
...
GeneticAlgorithm::GeneticAlgorithmResults* GeneticAlgorithm::perform_inputs_selection(void)
{
    ...
    while(!end)
    {
        ...
        current_inputs = population.arrange_row(current_minimal_generalization_error_index);
        ...
        if ((fabs(optimum_generalization_error - current_minimum_generalization_error) >= tolerance &&
             optimum_generalization_error > current_minimum_generalization_error) ||
                (fabs(optimum_generalization_error - current_minimum_generalization_error) < tolerance &&
                 optimal_inputs.count_occurrences(1) < current_inputs.count_occurrences(1)))
        {
            optimal_inputs = current_inputs;
        ...
        }else if (current_minimum_generalization_error <= generalization_performance_goal)
        {
            end = true;
            if (display)
        ...
        if (display)
        {
            ...
            std::cout << "Generation optimal number of inputs : " << current_inputs.count_occurrences(1) << std::endl;
...
tinyxml2::XMLDocument* GeneticAlgorithm::to_XML(void) const
{
    ...
    // Regression
    {
        element = document->NewElement("Regression");
    ...
    {
        element = document->NewElement("ReserveGenerationStandarDeviation");
        root_element->LinkEndChild(element);
    ...
    // Reserve generation minimum
    {
        element = document->NewElement("ReserveGenerationMinimum");
        root_element->LinkEndChild(element);

        buffer.str("");
        buffer << reserve_generation_minimum;
...
void GeneticAlgorithm::from_XML(const tinyxml2::XMLDocument& document)
{
    ...
    // Regression
    {
        const tinyxml2::XMLElement* element = root_element->FirstChildElement("Regression");
        ...
        if(element)
        {
            ...
            try
            {
                set_regression(new_regression != "0");
            }
    ...
    // Elitism size
    {
        const tinyxml2::XMLElement* element = root_element->FirstChildElement("ElistismSize");
    ...
    // Reserve generation minimum
    {
        const tinyxml2::XMLElement* element = root_element->FirstChildElement("ReserveGenerationMinimum");

        if(element)
        {
            const std::string new_reserve_generation_minimum = element->GetText();

            try
            {
                set_reserve_generation_minimum(new_reserve_generation_minimum != "0");
            }
            catch(const std::logic_error& e)
            {
                std::cout << e.what() << std::endl;
