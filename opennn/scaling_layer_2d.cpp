                                     ...
                                     type* outputs_data, const Tensor<Index, 1>& output_dimensions)
{
    ...
    if(input_rank == 2)
    {
        ...
        if(output_dimensions[0] != points_number || output_dimensions(1) != neurons_number)
            throw invalid_argument("Outputs dimensions must be equal to " + to_string(points_number) + " and " + to_string(neurons_number) + ".\n");
        ...
        for(Index i = 0; i < neurons_number; i++)
        {
            ...
            {
                throw invalid_argument("Unknown scaling method.\n");
            }
    ...
    else if(input_rank == 4)
    {
        ...
        if(!equal_dimensions(0))
            throw invalid_argument("Input and output data must have the same dimensions.\n");
    ...
    {
        throw invalid_argument("Input dimension must be 2 or 4.\n");
    }
}
...
string ScalingLayer2D::write_expression(const Tensor<string, 1>& input_names, const Tensor<string, 1>&) const
{
    ...
    for(Index i = 0; i < neurons_number; i++)
    {
        ...
        {
            buffer << "scaled_" << input_names(i) << " = " << input_names(i) << "*(" << max_range << "-" << min_range << ")/("
