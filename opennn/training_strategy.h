...
public:
  ...
  TrainingStrategy& operator=(const TrainingStrategy&)=delete;
   ...
   struct Results
   {
     ...
     std::unique_ptr<RandomSearch::RandomSearchResults> random_search_results_pointer;
        ...
        std::unique_ptr<StochasticGradientDescent::StochasticGradientDescentResults> stochastic_gradient_descent_results_pointer;
  };
  ...
  /// ownership not passed
   ...
   void perform_training(Results&) const;
  Results&& perform_training(void) const {
    Results r;
    perform_training(r);
    return std::move(r);
  }
