...
public:
    ...
    Vector<size_t> count_equal_to(const T&) const;
    ...
    Matrix<T> arrange_column_less_than(const size_t&, const T&, const bool&) const;
...
Matrix<T> Matrix<T>::arrange_submatrix(const Vector<size_t>& row_indices, const Vector<size_t>& column_indices) const
{
   ...
   Matrix<T> sub_matrix(row_indices_size, column_indices_size,0.0);
...
{
/*
    Vector<string> rows(rows_number);

    for(size_t i = 0; i < rows_number; i++)
    {
        rows[i] = arrange_row(i).vector_to_string(';');
    }

    const Vector<string> unique_rows = rows.arrange_unique();

    const size_t unique_rows_number = unique_rows.size();

    Matrix<T> unique(unique_rows_number, columns_number);

    Vector<T> row;

    for(size_t i = 0; i < unique_rows_number; i++)
    {
        row = unique_rows.split_element(i, ';');

        unique.set_row(i, row);
    }
...
*/
    ...
    unique.insert_row(0,(*this).arrange_row(0));
...
Matrix<T> Matrix<T>::remove_first_rows(const size_t& number) const
{
    ...
    const Vector<size_t> indices(number, 1, rows_number);
...
{
    const Vector<size_t> indices(0, 1, number);
...
Vector<double> Matrix<T>::calculate_mean_missing_values(const Vector<size_t>& row_indices, const Vector<size_t>& column_indices, const Vector< Vector<size_t> >& missing_indices) const
{
   ...
   for(int i = 0; i < (int)column_indices_size; i++)
   {
    ...
    #pragma omp parallel for private(row_index, column_index, count) schedule(dynamic)
                                                          ...
                                                          const Vector< Vector<size_t> >& missing_indices) const
{
   ...
   for(int i = 0; i < (int)column_indices_size; i++)
   {
    ...
    #pragma omp parallel for private(column_index) schedule(dynamic)
...
Vector< Statistics<T> > Matrix<T>::calculate_statistics(const Vector< Vector<size_t> >& row_indices, const Vector<size_t>& column_indices) const
{
    ...
    size_t row_index = 0, column_index = 0;
...
#pragma omp parallel for private(row_index, column_index)
...
Matrix<T> Matrix<T>::fill_missing_dates_dd_mm_yyyy(const string& column_name, const char& separator) const
{
    ...
    Matrix<T> data(*this);
    data.convert_time_column_dd_mm_yyyy(column_name, '/', true);
...
template <class T>
Matrix<T> Matrix<T>::arrange_column_less_than(const size_t& column_index, const T& value, const bool& header) const
{
    const Vector<T> column = arrange_column(column_index);

    size_t new_rows_number = rows_number - column.count_less_than(value);
...
template <class T>
void Matrix<T>::convert_time_series(const size_t& lags_number, const size_t& column_index)
{
    Vector<T> column = arrange_column(column_index);

    remove_column(column_index);

    Vector<size_t> indices(lags_number, 1, rows_number - 1);

    Vector<T> first_column = column.arrange_subvector(indices);
