...
KNearestNeighbors::KNearestNeighbors()
{
    ...
    weights.set(data_set_pointer->get_data().get_columns_number(),1.0);
...
void KNearestNeighbors::set_normalization_method(const Method& new_method)
{
    normalization_method = new_method;
}
...
void KNearestNeighbors::set_normalization_method(const string& new_method_string)
{
...
Matrix<double> KNearestNeighbors::calculate_correlation_weights(void) const
{
    ...
    return correlations.calculate_softmax_columns(); // @todo: check method
}
...
Matrix<double> KNearestNeighbors::calculate_output_data(const Matrix<double>& input_data) const
{
    ...
    const size_t targets_number = data_set_pointer->get_variables().count_targets_number();
...
{
    return calculate_output_data(data_set_pointer->arrange_selection_input_data());
}
...
{
    return calculate_output_data(data_set_pointer->arrange_testing_input_data());
}
...
{
    const Neighbors k_nearest_neighbors = calculate_k_nearest_neighbors(inputs);
...
KNearestNeighbors::Neighbors KNearestNeighbors::calculate_k_nearest_neighbors(const Vector<double>& inputs) const
{
    const Matrix<double> training_inputs = data_set_pointer->arrange_training_input_data();
    ...
    {
        const Vector<double> current_distances = training_inputs.calculate_weighted_distance(inputs, weights.get_column(i));

        const Vector<size_t> nearest_neighbors = current_distances.calculate_lower_indices(k);

        const Vector<double> nearest_distances = current_distances.calculate_lower_values(k);
...
{
    const Matrix<double> targets = data_set_pointer->arrange_training_target_data();
...
{
    const Matrix<double> selection_target_data = data_set_pointer->arrange_selection_target_data();
...
{
    const size_t instances_number = data_set_pointer->get_instances_pointer()->count_used_instances_number();
