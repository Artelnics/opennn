...
string ModelExpression::subheader_javascript() const
{
    return
        ...
        "}\n\n"
        "/* Categorical inputs use a <select> combo box; its category labels read\n"
        "   left-to-right, so left-align them (numeric value fields stay right). */\n"
        ".neural-cell select {\n"
        "text-align: left;\n"
...
string ModelExpression::get_expression_python(const vector<Variable>& variables) const
{
    ...
    vector<string> original_outputs = neural_network->get_output_feature_names();
