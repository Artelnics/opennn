    ...
    TensorView& output)
{
    ...
    const VectorMap rmean = running_mean.as_vector();
    const VectorMap rvar = running_variance.as_vector();

    output_matrix = ((input_matrix.rowwise() - rmean.transpose()).array()
                     .rowwise() / (rvar.array() + EPSILON).sqrt().transpose())
