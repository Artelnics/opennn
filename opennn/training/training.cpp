...
Training::Training(Network* new_network, Dataset* new_dataset)
{
    set(new_network, new_dataset);
}

void Training::set(Network* new_network, Dataset* new_dataset)
{
    network = new_network;
    dataset = new_dataset;
...
void Training::set_dataset(Dataset* new_dataset)
{
    dataset = new_dataset;
    if (loss) loss->set_dataset(new_dataset);
}

void Training::set_network(Network* new_network)
{
    network = new_network;
    if (!network)
    {
...
void Training::set_loss(const string& new_loss)
{
...
void Training::set_optimization_algorithm(const string& new_optimization_algorithm)
{
...
void Training::to_JSON(JsonWriter& printer) const
{
    throw_if(!loss || !optimizer,
             "Training::to_JSON: the loss and the optimizer must be set.");

    printer.open_element("Training");
...
void Training::from_JSON(const JsonDocument& document)
{
    const Json* root_element = get_json_root(document, "Training");
...
void Training::save(const filesystem::path& file_name) const
{
    save_json_file(file_name, *this);
}

void Training::load(const filesystem::path& file_name)
{
    set_default();
