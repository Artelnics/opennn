...
LevenbergMarquardt::LevenbergMarquardt(Loss* new_loss)
    : Optimizer(new_loss)
{
...
void LevenbergMarquardt::back_propagate(const Batch& batch,
...
void LevenbergMarquardt::calculate_errors(const Batch& batch,
                                                   const ForwardPropagation& forward_propagation,
                                                   BackPropagationLM& back_propagation_lm) const
{
    ...
    throw_if(output.size() != target.size() || output.size() != back_propagation_lm.errors.size(),
             "LevenbergMarquardt: outputs ({}), targets ({}) and errors ({}) sizes do not match. The dataset target count does not match the network outputs.", output.size(), target.size(), back_propagation_lm.errors.size());
...
void LevenbergMarquardt::compute_jacobian(const Batch&  ,
                                                   const ForwardPropagation& forward_propagation,
                                                   BackPropagationLM& back_propagation_lm) const
{
...
TrainingResult LevenbergMarquardt::train()
{
    Network* network = loss->get_network();
    ...
    hooks.validation_error = [&]
    {
        ...
        throw_if(output.size() != target.size(),
                 "LevenbergMarquardt: validation outputs ({}) and targets ({}) sizes do not match.",
...
void LevenbergMarquardt::update_full_batch_parameters(const Batch& batch,
...
void LevenbergMarquardt::to_JSON(JsonWriter& printer) const
{
...
void LevenbergMarquardt::from_JSON(const JsonDocument& document)
{
