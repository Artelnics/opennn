...
QuasiNewton::QuasiNewton(Loss* new_loss)
    : Optimizer(new_loss)
{
    name = "QuasiNewton";
...
void QuasiNewton::calculate_inverse_hessian(OptimizerData& optimization_data) const
{
...
void QuasiNewton::update_full_batch_parameters(const Batch& batch,
...
TrainingResult QuasiNewton::train()
{
    Network* network = loss->get_network();
...
void QuasiNewton::to_JSON(JsonWriter& printer) const
{
    printer.open_element("QuasiNewton");
...
void QuasiNewton::from_JSON(const JsonDocument& document)
{
    const Json* root_element = get_json_root(document, "QuasiNewton");

    set_minimum_loss_decrease(read_json_float(root_element, "MinimumLossDecrease"));
    read_common_json(root_element);
}

pair<float, float> QuasiNewton::calculate_directional_point(
    const Batch& batch,
    ForwardPropagation& forward_propagation,
    BackPropagation& back_propagation,
    float current_loss)
