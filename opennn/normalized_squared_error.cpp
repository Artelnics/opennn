...
LossIndex::FirstOrderLoss NormalizedSquaredError::calculate_batch_first_order_loss(const DataSet::Batch& batch) const
{
#ifdef __OPENNN_DEBUG__
...
check();
    ...
    const Vector<Layer::FirstOrderActivations> forward_propagation = neural_network_pointer->calculate_trainable_forward_propagation(batch.inputs);
    ...
    const Vector<double> batch_gradient = calculate_error_gradient(batch.inputs, forward_propagation, layers_delta);
