...
// Same pattern, but for backward gradient writes. cuDNN's
// cudnnConvolutionBackwardFilter / BackwardBias require their dwDesc / dbDesc
// to share dtype with x/dy (BF16 in BP16 mode), but the master gradient buffer
// is FP32. Conv backward writes the BF16 gradient here, then a tiny
// cast_bf16_to_fp32_cuda kernel merges it into the FP32 master.
__nv_bfloat16* get_bf16_grad_scratch(Index n_elements);

// FP32 scratch on device, used to up-cast a BF16 tensor before passing it to
// a cuDNN op that doesn't support BF16 (notably cudnnConvolutionBackwardBias,
