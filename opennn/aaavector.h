...
public:
  ...
  void randomize_normal(const Eigen::Tensor<double, 1>&, const Eigen::Tensor<double, 1>&);
  ...
  Eigen::Tensor<double, 1> perform_Box_Cox_transformation(const double& = 1) const;

  // Descriptives methods

  Eigen::Tensor<double, 1> calculate_percentage(const size_t&) const;
  Eigen::Tensor<double, 1> calculate_percentage() const;
  ...
  Eigen::Tensor<double, 1> to_double_vector() const;
  ...
  Eigen::Tensor<double, 1> string_to_double() const;
                                  ...
                                  const Vector<T>&maximums)
{
  ...
  if(minimums > maximums) {
    ...
    buffer << "OpenNN Exception: Vector Template.\n"
           << "void randomize_uniform(const Eigen::Tensor<double, 1>&, const "
              "Eigen::Tensor<double, 1>&) method.\n"
...
template <class T>
void Vector<T>::randomize_normal(const Eigen::Tensor<double, 1>&mean,
                                 const Eigen::Tensor<double, 1>&standard_deviation)
{
  const size_t this_size = this->size();
  ...
  if(mean_size != this_size || standard_deviation_size != this_size) {
    ...
    buffer
        ...
        << "void randomize_normal(const Eigen::Tensor<double, 1>&, const "
           "Eigen::Tensor<double, 1>&) method.\n"
...
template <class T> Eigen::Tensor<double, 1> Vector<T>::perform_Box_Cox_transformation(const double& lambda) const
{
    const size_t size = this->size();

    Eigen::Tensor<double, 1> vector_tranformation(size);
...
template <class T>
Eigen::Tensor<double, 1> Vector<T>::calculate_percentage(const size_t& total_sum) const
{
    const size_t this_size = this->size();

    Eigen::Tensor<double, 1> percentage_vector(this_size);
...
template <class T> Eigen::Tensor<double, 1> Vector<T>::calculate_percentage() const
{
    const size_t this_size = this->size();
...
void Vector<T>::print_unique_string() const
{
    ...
    const Eigen::Tensor<double, 1> percentage = count_unique().to_double_vector()*(100.0/static_cast<double>(this_size));
...
Vector<T> Vector<T>::calculate_top_number(const size_t& rank) const
{
    ...
    const Eigen::Tensor<double, 1> count_double = count.to_double_vector();
...
template <class T>
Eigen::Tensor<double, 1> Vector<T>::to_double_vector() const
{
  const size_t this_size = this->size();

  Eigen::Tensor<double, 1> double_vector(this_size);
...
template <class T>
Eigen::Tensor<double, 1> Vector<T>::string_to_double() const
{
  const size_t this_size = this->size();
