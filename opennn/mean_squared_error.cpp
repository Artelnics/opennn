...
double MeanSquaredError::calculate_training_error(const Tensor<double, 1>& parameters) const
{
#ifdef __OPENNN_DEBUG__
...
/*
    ...
    for(int i = 0; i < static_cast<int>(batches_number); i++)
    {
        const Tensor<double, 2> inputs = data_set_pointer->get_input_data(training_batches[static_cast<int>(i)]);
        const Tensor<double, 2> targets = data_set_pointer->get_target_data(training_batches[static_cast<int>(i)]);
        ...
        const vector<Tensor<double, 2>> layers_delta
...
check();
    ...
    const vector<Layer::ForwardPropagation> forward_propagation = neural_network_pointer->calculate_forward_propagation(batch.inputs_2d);

    const Tensor<double, 2> output_gradient = calculate_output_gradient(forward_propagation[layers_number-1].activations, batch.targets_2d);

    const vector<Tensor<double, 2>> layers_delta = calculate_layers_delta(forward_propagation,
                                                                       output_gradient);

    const Tensor<double, 1> batch_error_gradient = calculate_error_gradient(batch.inputs_2d, forward_propagation, layers_delta);
/*
...
Tensor<double, 2> MeanSquaredError::calculate_output_gradient(const Tensor<double, 2>& outputs, const Tensor<double, 2>& targets) const
{
#ifdef __OPENNN_DEBUG__
...
Tensor<double, 1> MeanSquaredError::calculate_training_error_terms(const Tensor<double, 2>& outputs, const Tensor<double, 2>& targets) const
{
   // Control sentence
...
Tensor<double, 1> MeanSquaredError::calculate_training_error_terms(const Tensor<double, 1>& parameters) const
{
#ifdef __OPENNN_DEBUG__
...
/*
        ...
        Tensor<double, 2> hessian_approximation;// = error_terms_Jacobian.dot(error_terms_Jacobian);
