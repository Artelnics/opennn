...
public:
    ...
    Eigen::Tensor<double, 1> count_equal_to_by_rows(const T&, const Eigen::Tensor<double, 1>&) const;
    ...
    void randomize_uniform(const Eigen::Tensor<double, 1>&, const Eigen::Tensor<double, 1>&);
    ...
    Eigen::Tensor<double, 1> calculate_missing_values_percentage() const;
...
template <class T>
Eigen::Tensor<double, 1> Matrix<T>::count_equal_to_by_rows(const T& value, const Eigen::Tensor<double, 1>& weights) const
{
    Eigen::Tensor<double, 1> count_by_rows(rows_number);
...
Matrix<T> Matrix<T>::sort_ascending_strings(const size_t& column_index) const
{
    ...
    const Eigen::Tensor<double, 1> column = get_column(column_index).string_to_double();
...
template <class T>
void Matrix<T>::randomize_uniform(const Eigen::Tensor<double, 1>& minimums, const Eigen::Tensor<double, 1>& maximums)
{
   ...
   if(minimums.size() != columns_number)
   {
      ...
      buffer << "OpenNN Exception: Matrix Template.\n"
             << "void randomize_uniform(const Eigen::Tensor<double, 1>&, const Eigen::Tensor<double, 1>&) const method.\n"
   ...
   Eigen::Tensor<double, 1> column(rows_number);
...
template <class T>
void Matrix<T>::randomize_normal(const Eigen::Tensor<double, 1>& means, const Eigen::Tensor<double, 1>& standard_deviations)
{
   ...
   if(means.size() != columns_number)
   {
      ...
      buffer << "OpenNN Exception: Matrix Template.\n"
             << "void randomize_normal(const Eigen::Tensor<double, 1>&, const Eigen::Tensor<double, 1>&) const method.\n"
...
template <class T>
Eigen::Tensor<double, 1> Matrix<T>::calculate_missing_values_percentage() const
{
    Eigen::Tensor<double, 1> missing_values(columns_number);
...
Matrix<T> Matrix<T>::insert_padding(const size_t& width, const size_t& height) const
{
    ...
    for(size_t i = 0; i < width; i++)
    {
        ...
        const Eigen::Tensor<double, 1> zero_vector_rows(rows_number, 0.0);
    ...
    for(size_t i = 0; i < height; i++)
    {
        ...
        const Eigen::Tensor<double, 1> zero_vector_columns(columns_number, 0.0);
