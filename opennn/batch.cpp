...
void Batch::set(const Index new_samples_number, const Dataset* new_dataset)
{
    ...
    {
        TensorView in_view(input.as<float>(), input_shape, CUDNN_DATA_FLOAT);
...
void Batch::copy_device_async(const Index current_batch_size, cudaStream_t stream)
{
    ...
    CHECK_CUDA(cudaMemcpyAsync(input.as<float>(), inputs_host, input_size * sizeof(float), cudaMemcpyHostToDevice, stream));
