...
[Release changes](#900-unreleased-candidate) · [Migrating from 8.x](#migrating-from-8x-to-90)
...
- Rename the public class to `Network` without an alias, move the network headers
  ...
  see [the migration guide](#migrating-from-8x-to-90) before rebuilding consumers or loading earlier models.
...
does not establish compatibility with historical model files. See [the migration guide](#migrating-from-8x-to-90);
...
trained models in [the reproduction recipes](DATASETS.md#reproduction) likewise test
