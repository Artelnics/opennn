...
RESULTS_DIR = (HERE.parent / "results").resolve()
...
def main() -> None:
    ...
    out_path = Path(args.output).resolve() if args.output else RESULTS_DIR / f"gpu-dense-higgs-training-speed-{run_id}.json"

    git = git_metadata()
    result: dict[str, Any] = {
        "schema_version": 1,
        "benchmark_id": "gpu-dense-higgs-training-speed",
        "run_id": run_id,
