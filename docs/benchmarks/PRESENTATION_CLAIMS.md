...
| Lead now | Peak memory | On the same small training job, OpenNN peaks around 9 MB RSS, versus 295 MB for PyTorch and 521 MB for TensorFlow. | [Peak memory](peak-memory-opennn-vs-pytorch-vs-tensorflow.md) | Attach raw RSS output and process measurement method. | The model is intentionally small to isolate fixed runtime overhead. |
