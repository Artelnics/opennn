#!/usr/bin/env bash
# Run the four CPU inference-speed benchmarks at a shared config and print a
# comparison. All four run the same 2-layer MLP (F -> F -> 1, tanh then linear)
...
# work). ONNX Runtime loads a model exported once by export_onnx.py.
#
...
ONNX_MODEL="$HERE/model_inference.onnx"
...
echo "==== exporting ONNX model (once) ===="
"$VENV" "$HERE/export_onnx.py" "$FEATURES" "$ONNX_MODEL"
echo
...
r=$("$VENV" "$HERE/onnxruntime_inference.py" "$SAMPLES" "$FEATURES" "$BATCH" "$REPS" "$ONNX_MODEL" 2>/dev/null)
echo "$r" | grep -E 'samples_per_sec|median_pass_s|ms_per_batch|RESULT'
ORT=$(echo "$r" | extract samples_per_sec)
