...
`docs/benchmarks/resnet50-training-speed/prepare_cifar10.py`. The OpenNN trial
uses `ImageDataset` in its default `BinaryFile` storage mode, so pixels are read
from the image cache file for the batch. It does not enable
`OPENNN_GPU_RESIDENT_DATA`.
