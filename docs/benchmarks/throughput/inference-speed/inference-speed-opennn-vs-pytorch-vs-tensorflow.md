# Inference speed: OpenNN vs PyTorch vs TensorFlow (CPU)
...
The [startup-latency benchmark](../../footprint/startup/startup-latency-opennn-vs-pytorch-vs-tensorflow.md)
...
Inference is the workload all three can be compared on directly: once a model is trained, running
its forward pass is exactly what each framework does at deployment time.
...
All three run single-machine CPU, FP32, batching the dataset and reporting the median over many
...
The Python engines were measured in a temporary Python 3.13 venv with `torch 2.10.0+cpu` and
`tensorflow 2.21.0`. Competitors were also checked with their best observed thread settings on
this machine: PyTorch `MKL_NUM_THREADS=28`, TensorFlow default.

| | OpenNN + MKL | PyTorch | TensorFlow |
|---|---:|---:|---:|
| **Throughput (samples/sec)** | **466,837** | 372,780 | 375,409 |
| **Latency (ms/batch of 1000)** | **2.14** | 2.68 | 2.66 |
| vs OpenNN | 1x | 0.80x | 0.80x |
...
* The `[fast_vml 1]` argument (set in code via `cpu_math::set_mkl_fast_vml`) uses MKL's enhanced-performance tanh mode. Accuracy parity is covered
  separately in the [accuracy benchmark](../../quality/accuracy/accuracy-opennn-vs-pytorch-vs-tensorflow.md).

## Reproducing

The equivalent programs are in [`docs/benchmarks/throughput/inference-speed/`](inference-speed/):
`opennn_inference.cpp`, `pytorch_inference.py`, and `tensorflow_inference.py`.
...
# Run all three at the article's config (csv samples features batch reps).
docs/benchmarks/throughput/inference-speed/run_inference.sh rosenbrock.csv 8000 1000 1000 30
```
