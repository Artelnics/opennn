...
//
// Written the way a user of the library would write it: load the CSV with
// TabularDataset, build the network, train or run the forward, report. The
// measurement protocol - arm rotation, soaking, medians over rounds - lives in
// run_higgs_cpu_sweep.py, not here.
//
// Modes:
//   opennn_higgs_cpu train <train_csv> <test_csv> [epochs] [batch[,batch...]] [hidden] [hidden_layers] [activation] [warmup_epochs]
//   opennn_higgs_cpu infer <test_csv> [reps] [batch[,batch...]] [hidden] [hidden_layers] [activation]
//
// A comma-separated batch list is measured inside one process so the batch
// sizes share one data load and one thermal window; each batch size prints its
// own batch_<B>_... lines, which is what the sweep runner reads.
...
vector<Index> parse_batches(const string& text)
{
    ...
    for (size_t start = 0; start < text.size();)
    {
        ...
        const size_t end = comma == string::npos ? text.size() : comma;
                                       ...
                                       const string& activation)
{
    ...
    for (Index i = 0; i < hidden_layers; ++i)
    {
        network->add_layer(make_unique<opennn::Dense>(
            current, Shape{hidden}, hidden_activation, false,
            "higgs_dense_" + to_string(i + 1)));
        current = network->get_output_shape();
    }

    network->add_layer(make_unique<opennn::Dense>(
        current, target_shape, "Sigmoid", false, "higgs_output"));
...
BinaryMetrics evaluate(NeuralNetwork& network, const TabularDataset& dataset, Index batch)
{
    ...
    for (Index i = 0; i + batch <= samples; i += batch)
    {
        ...
        for (Index r = 0; r < batch; ++r)
        {
            ...
            correct += (probability >= 0.5f ? 1 : 0) == label ? 1 : 0;
            log_loss += label ? -log(double(probability))
...
int train_mode(int argc, char* argv[])
{
    ...
    for (const Index batch : batches)
    {
        ...
        // The strategy owns the epoch loop, so one call covers all the epochs
        // and only their mean is observable from user code.
        const double epoch_s = chrono::duration<double>(t1 - t0).count() / double(epochs);
        const BinaryMetrics metrics = evaluate(*network, test_dataset, batch);
...
int infer_mode(int argc, char* argv[])
{
    if (argc < 3)
    {
        cerr << "usage: opennn_higgs_cpu infer <test_csv> [reps] [batch[,batch...]] [hidden] [hidden_layers] [activation]\n";
        return 2;
    }
    ...
    for (const Index batch : batches)
    {
        ...
        // In temporal order, before the sort: the sweep runner keeps these so a
        // drifting machine shows in the data instead of being averaged away.
        ...
        if (batches.size() == 1)
        {
            ...
            cout << "samples_per_sec=" << long(double(processed) / median_pass_s) << "\n";
        }
