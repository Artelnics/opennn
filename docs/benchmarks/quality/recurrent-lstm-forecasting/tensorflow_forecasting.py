...
pytorch_forecasting.py, so the three engines (OpenNN / PyTorch / TensorFlow) are
directly comparable on aggregated test RMSE and training throughput.

RMSE convention: standard sqrt(mean((pred-true)^2)), matching pytorch_forecasting.py
and the OpenNN C++ driver (errs(2)*sqrt(2)).

  usage: tensorflow_forecasting.py [--allow-cpu] [B1 B2 ...]   (default: all)
