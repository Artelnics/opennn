...
ONNX Runtime's CUDA execution provider works well on Windows — but ONNX Runtime is an
**inference engine**. It runs a model that was *already trained* and exported elsewhere; it does
not train and does not define a model. For the GPU-training question it is simply the wrong tool,
which is why it is marked n/a above. It returns as a real contender in the inference section
below.
...
For *inference* — running a trained model — the picture changes, and ONNX Runtime belongs in it.
ONNX Runtime's CUDA provider is a strong, well-supported Windows inference path and does **not**
share PyTorch's compile gap or TensorFlow's missing-GPU problem. The relevant comparison there is
...
[deployment benchmarks](README.md) for those head-to-heads (including ONNX Runtime).

The one caveat that carries over: to use ONNX Runtime at all you must first **export** a model to
the ONNX format from a training framework, and that conversion has its own friction (operator
coverage, opset versions, dynamic shapes). OpenNN trains and infers the same native model
end-to-end with no export step.
...
* The limitations described are platform facts (Triton/`torch.compile` on Windows; TensorFlow
  native GPU dropped after 2.10; ONNX Runtime being inference-only), not measurements — they hold
...
* ONNX Runtime is included on the inference axis, where it is directly comparable, and marked n/a
  for training, where it does not compete — the same convention the other benchmark notes use.
