...
| **Runs with no ML runtime?** | **yes** (C export needs only a C compiler) | no — needs libtorch | no — needs the TF / TFLite runtime |
...
or another inference engine for the portable ONNX format — to execute.
...
- **ONNX** (`torch.onnx.export` → `.onnx`): a portable model format that runs under an inference
