...
def engine_cmd(engine, dataset, epochs, batch, bf16):
    """Return (cmd, env_overrides). OpenNN reads <dataset>/train (BMPs); the
    Python engines read <dataset>/ (npy)."""
...
def main():
    ...
    for prec in precisions:
        ...
        per_prec = {}
        for eng in engines:
            cmd, env_over = engine_cmd(eng, args.dataset, args.epochs, args.batch, bf16)
            vals = []
            last_raw = ""
