#!/usr/bin/env python3
"""Max training batch + training speed for the chat Transformer, GPU.

OpenNN vs PyTorch vs TensorFlow, fp32 and bf16, CUDA graph OFF (does not help
the transformer path). Four numbers per framework:
    max_batch fp32, max_batch bf16, speed fp32 (samples/s), speed bf16.
...
CORPUS = os.environ.get("CHAT_CORPUS", "/home/artelnics/Documents/datasets/chat/chat_pairs.txt")
...
def cmd_env(engine, precision, batch, steps, shape):
...
def run_trial(engine, precision, batch, steps, shape, cap_mib):
    cmd, env = cmd_env(engine, precision, batch, steps, shape)
    try:
...
def search_max_batch(engine, precision, shape, cap_mib):
    cache = {}
    def trial(b):
        if b not in cache:
            cache[b] = run_trial(engine, precision, b, args.probe_steps, shape, cap_mib)
            r = cache[b]
            print(f"  {engine:11s} {precision} batch={b:<6d} "
...
def measure_speed(engine, precision, batch, shape, cap_mib):
    # Steady-state throughput. PyTorch/TF time only their bare step loop, but
    # OpenNN's wall_s covers the whole train() call, which includes a fixed
    # warmup (cuDNN plan selection, allocations) and teardown (parameter D2H,
    # frees) inside the timed window. Two-point differencing -- run 1x and 4x
    # the steps and divide the extra samples by the extra wall time -- cancels
    # that fixed cost exactly, so all engines report the same quantity.
    long_steps = args.speed_steps * 4
    if engine == "opennn":
        short = run_trial(engine, precision, batch, args.speed_steps, shape, cap_mib)
        cooldown()
        long = run_trial(engine, precision, batch, long_steps, shape, cap_mib)
        cooldown()
    ...
    r = run_trial(engine, precision, batch, long_steps, shape, cap_mib)
    cooldown()
...
def main():
    ...
    ap.add_argument("--max-limit", type=int, default=8192)
    ...
    print(f"GPU total={total_mib} MiB, cap={cap_mib} MiB, speed_batch={args.speed_batch}")
    ...
    for e in engines:
        for p in precisions:
            print(f"\n[max-batch] {e} {p}")
            mb, peak = search_max_batch(e, p, shape, cap_mib)
            print(f"[speed]     {e} {p} @batch {args.speed_batch}")
            sps = measure_speed(e, p, args.speed_batch, shape, cap_mib)
            results[(e, p)] = {"max_batch": mb, "peak_at_max": peak, "speed_sps": sps}

    print("\n================= SUMMARY =================")
    print(f"{'engine':12s} {'prec':5s} {'max_batch':>10s} {'speed(sps)':>12s} {'peak@max(MiB)':>14s}")
    for e in engines:
        for p in precisions:
            r = results[(e, p)]
            print(f"{e:12s} {p:5s} {r['max_batch']:>10d} "
