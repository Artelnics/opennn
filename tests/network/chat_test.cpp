...
void make_tiny_decoder(Network& network,
...
void make_constant_tiny_decoder(Network& network,
                                Index sequence_length,
                                Index vocabulary_size,
...
TEST(ChatSessionTest, TrimReleasesDraftAndAllowsReattachment)
{
    ...
    Configuration::instance().set(Device::CUDA, Type::BF16);
    const TemplateTokenizer tokenizer;
    Network network, draft;
    for (Network* model : {&network, &draft})
    {
...
TEST(ChatSessionTest, SpeculativeGreedyFullAcceptanceMatchesBaseline)
{
    ...
    Network speculative_network;
    Network matching_draft;
...
TEST(ChatSessionTest, SpeculativeDraftRejectsInvalidConfiguration)
{
    ...
    Network valid_draft;
    Network short_draft;
    Network wrong_vocabulary_draft;
    make_constant_tiny_decoder(
