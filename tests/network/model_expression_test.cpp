...
protected:
    ...
    void SetUp() override
    {
        network = make_unique<ApproximationNetwork>(Shape{ 2 }, Shape{ 3 }, Shape{ 1 });
        ...
        network->set_input_names(input_names);
        network->set_output_names(output_names);
    }
};
...
{
    network->set_input_variables({Variable("kind <&>\"", "Input",
    ...
    ModelExpression(network.get()).save(path, ModelExpression::ProgrammingLanguage::JavaScript);
...
TEST_F(ModelExpressionTest, SavePythonExpression)
{
    ...
    EXPECT_TRUE(contains_token(source, "class Network"));
...
TEST_F(ModelExpressionTest, SaveJavaScriptExpression)
{
    ...
    EXPECT_FALSE(source.empty());
    EXPECT_TRUE(contains_token(source, "function network"));
...
protected:
    ...
    unique_ptr<ClassificationNetwork> network;
    ...
    void SetUp() override
    {
        network = make_unique<ClassificationNetwork>(Shape{ 2 }, Shape{ 3 }, Shape{ 3 });
        network->set_input_variables(vector<Variable>(network->get_inputs_number()));
        network->set_output_variables(vector<Variable>(network->get_outputs_number()));
        network->set_input_names(input_names);
        network->set_output_names(output_names);
