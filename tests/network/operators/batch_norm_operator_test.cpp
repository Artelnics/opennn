...
TEST(BatchNormalizationOperatoreratorTest, ForwardTrainingNormalizesPerFeature)
{
    ...
    Network network;
    network.add_layer(make_unique<opennn::Dense>(Shape{inputs_number}, Shape{outputs_number}, "Identity", BatchNormalization::Yes));
    network.compile();
...
TEST(BatchNormalizationOperatoreratorTest, RunningVarianceUsesSampleEstimate)
{
    ...
    Network network;
    network.add_layer(make_unique<opennn::Dense>(Shape{features}, Shape{features}, "Identity", BatchNormalization::Yes));
    network.compile();
    network.set_parameters_random();
    ...
    const VectorMap states_before(network.get_states_data(), network.get_states_size());
    ...
    const VectorMap states(network.get_states_data(), network.get_states_size());
...
TEST(BatchNormalizationOperatoreratorTest, GradientMatchesFiniteDifferences)
{
    ...
    Network network;
    network.add_layer(make_unique<opennn::Dense>(Shape{ inputs_number },
