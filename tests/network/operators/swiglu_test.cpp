...
TEST(SiLUTest, ActivationForwardMatchesSilu)
{
    ...
    Network network;
    network.add_layer(make_unique<Activation>(Shape{features}, "SiLU", "act"));
    network.compile();
...
TEST(SiLUTest, ActivationGradientMatchesNumerical)
{
    ...
    network.add_layer(make_unique<opennn::Dense>(Shape{features}, Shape{hidden}, "SiLU"), {-1});
    network.add_layer(make_unique<opennn::Dense>(Shape{hidden}, Shape{targets_number}, "Identity"));
    network.compile();
...
float gated_dense_max_gradient_error(bool use_bias)
{
    ...
    const Index gated_index = network.add_layer(std::move(gate_up), {-1});

    network.add_layer(make_unique<Flatten>(network.get_layer(gated_index)->get_output_shape()), {gated_index});
...
TEST(GatedDenseTest, SaveLoadRoundTrip)
{
    ...
    Network network;
    network.add_layer(std::move(gate_up));
    network.compile();
    network.set_parameters_random();
