...
TEST(Pool3dOperatoreratorTest, ForwardMaxValuesAndShape)
{
    ...
    Network network;
    network.add_layer(make_unique<Pooling3d>(input_shape, PoolingMethod::MaxPooling, "max_pool"));
    network.compile();
...
TEST(Pool3dOperatoreratorTest, ForwardAverageValuesWithPadding)
{
    ...
    Network network;
    network.add_layer(make_unique<Pooling3d>(input_shape, PoolingMethod::AveragePooling, "avg_pool"));
    network.compile();
...
TEST(Pool3dOperatoreratorTest, BackPropagateMaxGradient)
{
    ...
    Network network;
    network.add_layer(make_unique<Pooling3d>(Shape{seq, feat}, PoolingMethod::MaxPooling, "max_pool"));
    const Shape pool_output_shape = network.get_layer(0)->get_output_shape();
    network.add_layer(make_unique<opennn::Dense>(pool_output_shape, dataset.get_target_shape()));
    network.compile();
...
TEST(Pool3dOperatoreratorTest, BackPropagateAverageGradient)
{
    ...
    Network network;
    network.add_layer(make_unique<Pooling3d>(Shape{seq, feat}, PoolingMethod::AveragePooling, "avg_pool"));
    const Shape pool_output_shape = network.get_layer(0)->get_output_shape();
    network.add_layer(make_unique<opennn::Dense>(pool_output_shape, dataset.get_target_shape()));
    network.compile();
    network.set_parameters_random();
...
void add_padded_average_pooling_stack(Network& network)
{
...
void shift_normalization(Network& network)
{
...
TEST(Pool3dOperatoreratorTest, AverageIgnoresPaddingBehindANonzeroShift)
{
    ...
    Network network;
    add_padded_average_pooling_stack(network);
    network.compile();
    network.set_parameters_random();
    shift_normalization(network);
