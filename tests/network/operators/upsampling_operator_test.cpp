...
TEST_F(UpsamplingOperatorTest, ForwardOutputShape)
{
    ...
    Network network;
    network.add_layer(make_unique<Upsampling>(input_shape, scale_factor));
    network.compile();
...
TEST_F(UpsamplingOperatorTest, ScaleFactorOneIsIdentity)
{
    ...
    Network network;
    network.add_layer(make_unique<Upsampling>(input_shape, 1));
    network.compile();
...
TEST_F(UpsamplingOperatorTest, BackPropagateDeltaAccumulation)
{
    ...
    const Shape flatten_input_shape = network.get_layer(0)->get_output_shape();
    network.add_layer(make_unique<Flatten>(flatten_input_shape));

    const Shape dense_input_shape = network.get_layer(1)->get_output_shape();
    network.add_layer(make_unique<opennn::Dense>(dense_input_shape, dataset.get_target_shape()));
    network.compile();
