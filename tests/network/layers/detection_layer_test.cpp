...
TEST(Detection, ForwardPropagateMatchesHandComputedValuesForKnownLogits)
{
    ...
    Network network;
    network.add_layer(make_unique<Detection>(Shape{grid, grid, channels}, anchors, "detection"));
    network.compile();
...
TEST(Detection, ForwardPropagateSigmoidClassActivation)
{
    ...
    Network network;
    network.add_layer(std::move(detection));
    network.compile();
...
TEST(Detection, SigmoidClassBackwardGradientMatchesNumerical)
{
    ...
    const Index conv_index = network.add_layer(make_unique<Convolutional>(input_shape,
    ...
    auto detection = make_unique<Detection>(network.get_layer(conv_index)->get_output_shape(), anchors, "detection");
    detection->set_class_activation(Detection::ClassActivation::Sigmoid);
    const Index detection_index = network.add_layer(std::move(detection), {conv_index});

    network.add_layer(make_unique<Flatten>(network.get_layer(detection_index)->get_output_shape()),
                             {detection_index});
    ...
    network.get_parameters_map().setConstant(0.1f);
