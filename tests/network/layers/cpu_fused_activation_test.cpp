...
vector<float> forward_once(Network& network,
                           float* input_data,
...
TEST(CpuFusedActivationTest, BiasFreeDenseAppliesRelu)
{
    ...
    const vector<float> outputs = forward_once(network, inputs.data(), Shape{1, 2});
...
TEST(CpuFusedActivationTest, WidthEightDenseAppliesGeluTanh)
{
    ...
    Network network;
    network.add_layer(make_unique<opennn::Dense>(Shape{2}, Shape{output_features}, "GELUTanh"));
    network.compile();
...
TEST(CpuFusedActivationTest, ConvolutionAppliesReluWithoutBatchNorm)
{
    ...
    network.get_parameters_map().setConstant(1.0f);
    ...
    const vector<float> outputs = forward_once(network, inputs.data(), Shape{1, 3, 3, 1});
