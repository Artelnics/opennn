...
TEST(ClampingTest, InputShapeChangeKeepsMethodAndBounds)
{
    ...
    Network network;
    network.add_layer(make_unique<Clamping>(Shape{features},
                                                   Clamping::ClampingMethod::NoClamping,
                                                   "my_clamping"));
    network.compile();

    Clamping* layer = static_cast<Clamping*>(network.get_layer(0).get());
    ...
    network.set_input_shape(Shape{features});
...
TEST(ClampingTest, RejectsSuccessors)
{
    ...
    EXPECT_THROW(network.add_layer(std::move(dense)), runtime_error);
    EXPECT_EQ(network.get_layers_number(), 1);
}
...
TEST(ClampingTest, ForwardPropagate)
{
    ...
    Network network;
    network.add_layer(make_unique<Clamping>(Shape{columns_number}));
    network.compile();
    ...
    ForwardPropagation forward_propagation(rows_number, &network);
