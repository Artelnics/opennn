...
TEST(Dense2dTest, ForwardValuesMatchHandComputed)
{
    ...
    Network network;
    network.add_layer(make_unique<opennn::Dense>(Shape{inputs_number}, Shape{outputs_number}, "Identity"));
    network.compile();

    Layer* dense_layer = network.get_layer(0).get();
    ...
    const MatrixR result = network.calculate_outputs(input_data);
...
TEST(Dense3dTest, SequenceForwardValuesMatchHandComputed)
{
    ...
    Network network;
    network.add_layer(make_unique<opennn::Dense>(Shape{sequence_length, input_embedding},
...
TEST(Dense, BatchNormForwardOrGradient)
{
    ...
    Network network;
    network.add_layer(make_unique<opennn::Dense>(Shape{inputs_number}, Shape{targets_number},
                                                        "Identity", BatchNormalization::Yes));
    network.compile();
    network.set_parameters_random();

    EXPECT_TRUE(static_cast<opennn::Dense*>(network.get_layer(0).get())->get_batch_normalization());
