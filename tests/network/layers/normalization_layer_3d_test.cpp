...
TEST(Normalization3dTest, ForwardMatchesHandComputedLayerNorm)
{
    ...
    Network network;
    network.add_layer(make_unique<Normalization3d>(Shape{seq, dim}, "norm"));
    network.compile();
...
TEST(Normalization3dTest, BackwardGradientMatchesNumerical)
{
    ...
    const Index norm_index = network.add_layer(make_unique<Normalization3d>(input_shape, "norm"), {-1});

    network.add_layer(make_unique<Flatten>(network.get_layer(norm_index)->get_output_shape()),
                             {norm_index});
...
TEST(Normalization3dTest, FusedResidualAddForward)
{
    ...
    Network network;
    network.add_layer(std::move(norm), {-1, -2});
    network.compile();
    network.set_parameters_random();
...
TEST(Normalization3dTest, FusedResidualAddAliasesSafeBranchDelta)
{
    ...
    Network network;
    const Index residual_index = network.add_layer(
                                     make_unique<opennn::Dense>(input_shape, Shape{3}, "Identity", BatchNormalization::No,
                                                                "residual"),
                                     {-1});

    const Index branch_index = network.add_layer(
                                   make_unique<opennn::Dense>(input_shape, Shape{3}, "Identity", BatchNormalization::No,
                                                              "branch"),
                                   {residual_index});
    ...
    const Index normalization_index = network.add_layer(
    ...
    network.add_layer(
        make_unique<opennn::Dense>(
            network.get_output_shape(), Shape{2}, "Identity"));
    ...
    const VectorR parameters = network.get_parameters_map();
...
TEST(Normalization3dTest, RMSForwardMatchesHandComputedRMSNorm)
{
    ...
    Network network;
    network.add_layer(std::move(norm));
    network.compile();
    network.set_parameters_random();
...
TEST(Normalization3dTest, RMSBackwardGradientMatchesNumerical)
{
    ...
    norm->set_method(NormalizationMethod::RMS);
    const Index norm_index = network.add_layer(std::move(norm), {-1});
...
TEST(Normalization3dTest, RMSSaveLoadRoundTrip)
{
    ...
    ForwardPropagation forward_before(batch_size, &network);
    vector<TensorView> inputs_before = make_inputs();
    network.forward_propagate(inputs_before, forward_before, ForwardPropagationMode::Inference);
...
TEST(Normalization3dTest, LoadsLegacyRMSLayerName)
{
    ...
    Json& layers = legacy_document.get_root()["Network"]["Layers"];
...
TEST(Normalization3dTest, FusedResidualAddGradientMatchesNumerical)
{
    ...
    norm->set_fuse_add(true);
    const Index norm_index = network.add_layer(std::move(norm), {dense_index, -1});
