...
TEST_P(ConvolutionalLayerTest, ForwardPropagate)
{
    ...
    network.get_parameters_map().setConstant(type(0.5));
    ...
    const Shape expected_output_dims = network.get_layer(0)->get_output_shape();
...
TEST_P(ConvolutionalLayerTest, BackwardGradientMatchesNumerical)
{
    ...
    const Shape conv_output_shape = network.get_layer(0)->get_output_shape();
    network.add_layer(make_unique<Flatten>(conv_output_shape));
    network.add_layer(make_unique<opennn::Dense>(network.get_layer(1)->get_output_shape(),
