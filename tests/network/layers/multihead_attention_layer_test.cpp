...
TEST(MultiHeadAttentionTest, ForwardSelfAttentionMatchesHandComputed)
{
    ...
    Network network;
    network.add_layer(make_unique<MultiHeadAttention>(Shape{ sequence_length, embedding_dimension }, heads_number));
    network.compile();

    set_identity_projections(network.get_layer(0).get(), embedding_dimension);
...
TEST(MultiHeadAttentionTest, CausalMaskForward)
{
    ...
    Network network;
    network.add_layer(std::move(mha));
    network.compile();
...
TEST(MultiHeadAttentionTest, CrossAttentionForwardOrGradient)
{
    ...
    Network network;
    network.add_layer(make_unique<MultiHeadAttention>(Shape{ query_sequence_length, embedding_dimension },
                                                             Shape{ source_sequence_length, embedding_dimension },
                                                             heads_number),
                             { -1, -2 });
    network.compile();

    EXPECT_EQ(network.get_layer(0)->get_output_shape(), (Shape{ query_sequence_length, embedding_dimension }));
...
TEST(MultiHeadAttentionTest, BackwardGradientMatchesNumerical)
{
    ...
    Network network;
    network.add_layer(make_unique<MultiHeadAttention>(input_shape, heads_number));
