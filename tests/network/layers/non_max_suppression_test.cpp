...
TEST(NonMaxSuppression, GpuStagingIsPropagationOwned)
{
    ...
    ForwardPropagation first(batch_size, &network,
                             ForwardPropagationMode::Inference);
    ForwardPropagation second(batch_size, &network,
                              ForwardPropagationMode::Inference);
    vector<TensorView> inputs{
        TensorView(device_input.data(), {batch_size, grid, grid, channels},
                   Type::FP32, Device::CUDA)};

    network.forward_propagate(inputs, first, ForwardPropagationMode::Inference);
    network.forward_propagate(inputs, second, ForwardPropagationMode::Inference);
