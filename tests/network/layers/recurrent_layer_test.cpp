...
TEST(RecurrentLayerTest, ForwardPropagateValues)
{
    ...
    {
        Network network;
        network.add_layer(make_unique<Recurrent>(Shape{time_steps, inputs_number}, Shape{outputs_number}, "Identity"));
        network.compile();

        network.get_parameters_map().setConstant(type(0.1));
    ...
    {
        Network network;
        network.add_layer(make_unique<Recurrent>(Shape{time_steps, inputs_number}, Shape{outputs_number}, "Tanh"));
        network.compile();
...
TEST(RecurrentLayerTest, BackwardGradientMatchesNumerical)
{
    ...
    network.add_layer(make_unique<Recurrent>(input_shape, Shape{outputs_number}, "Tanh"));
    network.add_layer(make_unique<opennn::Dense>(Shape{outputs_number}, Shape{targets_number}, "Identity"));
