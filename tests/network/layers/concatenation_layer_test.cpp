...
TEST_F(ConcatenationLayerTest, ForwardPropagateConcatenatesAlongChannels)
{
    ...
    Network network;
    network.add_layer(make_unique<Concatenation>(input_shape, per_input_channels, "concat"),
                             vector<Index>{ -1, -2 });
    network.compile();
...
TEST_F(ConcatenationLayerTest, ForwardPropagateThreeInputs)
{
    ...
    Network network;
    network.add_layer(make_unique<Concatenation>(Shape{ height, width, c0 },
...
TEST_F(ConcatenationLayerTest, ConcatBackwardGradientMatchesNumerical)
{
    ...
    const Index branch_a_index = network.add_layer(make_unique<Convolutional>(data_input_shape,
                                                                                     Shape{ 1, 1, in_channels, branch_a_channels },
                                                                                     "Identity", Shape{ 1, 1 }, "Same", BatchNormalization::Yes,
                                                                                     "branch_a"),
                                                          vector<Index>{ -1 });

    const Index branch_b_index = network.add_layer(make_unique<Convolutional>(data_input_shape,
                                                                                     Shape{ 1, 1, in_channels, branch_b_channels },
                                                                                     "Identity", Shape{ 1, 1 }, "Same", BatchNormalization::Yes,
                                                                                     "branch_b"),
                                                          vector<Index>{ -1 });

    const Index concat_index = network.add_layer(make_unique<Concatenation>(Shape{ in_height, in_width, branch_a_channels },
                                                                                   vector<Index>{ branch_a_channels, branch_b_channels },
                                                                                   "concat"),
                                                        vector<Index>{ branch_a_index, branch_b_index });

    network.add_layer(make_unique<Flatten>(network.get_layer(concat_index)->get_output_shape()),
