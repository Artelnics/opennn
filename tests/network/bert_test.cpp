...
TEST(BertTest, FirstTokenPoolGradientCheck)
{
    ...
    Network network;
    network.add_layer(make_unique<opennn::Dense>(Shape{seq, features_in}, Shape{features}, "Identity"));
    network.add_layer(make_unique<Pooling3d>(Shape{seq, features}, PoolingMethod::FirstToken));
    network.add_layer(make_unique<opennn::Dense>(Shape{features}, Shape{targets_number}, "Identity"));
    network.compile();
