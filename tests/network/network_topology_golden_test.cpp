...
void dump_topology(const Network& network, const string& case_name)
{
    ...
    file << case_name << "\n"
         ...
         << "parameter buffer: " << network.get_parameters_buffer_size() << "\n\n";
    ...
    for (size_t i = 0; i < layers.size(); ++i)
    {
        ...
        const vector<Index>& inputs = network.get_source_layers()[i];
