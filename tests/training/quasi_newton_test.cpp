...
class QuasiNewtonTest : public ::testing::Test
{
protected:
...
TEST_F(QuasiNewtonTest, DefaultConstructor)
{
    QuasiNewton quasi_newton;

    EXPECT_EQ(quasi_newton.get_loss() == nullptr, true);
}

TEST_F(QuasiNewtonTest, GeneralConstructor)
{
    Loss loss;
    QuasiNewton quasi_newton(&loss);

    EXPECT_TRUE(quasi_newton.get_loss() != nullptr);
}

TEST_F(QuasiNewtonTest, RecordsValidationErrorWithoutBackwardState)
{
    set_seed(31);
...
TEST_F(QuasiNewtonTest, BFGS_Update)
{
...
TEST_F(QuasiNewtonTest, TrainApproximationCPU)
{
    set_seed(1);
    ...
    QuasiNewton quasi_newton_short(&loss_short);
    ...
    QuasiNewton quasi_newton_long(&loss_long);
...
TEST_F(QuasiNewtonTest, TrainClassificationCPU)
{
...
TEST_F(QuasiNewtonTest, MinimumLossDecreaseConverges)
{
    set_seed(3);
    TabularDataset dataset(16, {2}, {1});
    dataset.set_data_random();
    ...
    quasi_newton.set_maximum_epochs(1);
    const type error_short = quasi_newton.train().get_training_error();
    quasi_newton.set_maximum_epochs(100);
    const type error_long = quasi_newton.train().get_training_error();

    EXPECT_LT(error_long, error_short);
}

TEST_F(QuasiNewtonTest, StoppingMaximumEpochs)
{
    set_seed(4);
    TabularDataset dataset(16, {2}, {1});
    dataset.set_data_random();
    ...
    loss.set_error(Loss::Error::MeanSquaredError);
    QuasiNewton quasi_newton(&loss);
    quasi_newton.set_maximum_epochs(5);
    quasi_newton.set_minimum_loss_decrease(0.0f);
    quasi_newton.set_display(false);

    const TrainingResult training_results = quasi_newton.train();
...
TEST_F(QuasiNewtonTest, StoppingLossGoal)
{
    set_seed(5);
    ...
    quasi_newton.set_loss_goal(training_loss_goal);
    quasi_newton.set_minimum_loss_decrease(0.0f);
    quasi_newton.set_maximum_epochs(10000);
    quasi_newton.set_maximum_time(1000.0);
    quasi_newton.set_display(false);
...
TEST_F(QuasiNewtonTest, StoppingMaximumTime)
{
    set_seed(6);
    TabularDataset dataset(16, {2}, {1});
    dataset.set_data_random();
    ...
    loss.set_error(Loss::Error::MeanSquaredError);
    QuasiNewton quasi_newton(&loss);
    quasi_newton.set_maximum_epochs(1000000);
    quasi_newton.set_minimum_loss_decrease(0.0f);
    quasi_newton.set_maximum_time(0.5);
    quasi_newton.set_display(false);
...
TEST_F(QuasiNewtonTest, Determinism)
{
    ...
    QuasiNewton quasi_newton_first(&loss_first);
    ...
    QuasiNewton quasi_newton_second(&loss_second);
...
TEST_F(QuasiNewtonTest, RepeatedTrainingResetsState)
{
    set_threads_number(1);
    set_seed(31);
...
// once per epoch anyway. Full-batch QuasiNewton reads loss_value on every
...
TEST_F(QuasiNewtonTest, BackPropagateLeavesRegularizationToTheOptimizer)
{
    set_seed(1);
